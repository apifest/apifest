...
public class AuthorizationServer {
    ...
    public AccessToken issueAccessToken(HttpRequest req) throws OAuthException {
        ...
        } else if (tokenRequest.getGrantType().equals(ServerConfig.getCustomGrantType())) {
    ...
    protected UserDetails authenticateUser(String username, String password, HttpRequest authRequest) throws AuthenticationException {
        ...
        if (ServerConfig.getUserAuthenticationClass() != null) {
            try {
                ua = ServerConfig.getUserAuthenticationClass().newInstance();
    ...
    protected UserDetails callCustomGrantTypeHandler(HttpRequest authRequest) throws AuthenticationException {
        ...
        if (ServerConfig.getCustomGrantTypeHandler() != null) {
            try {
                customHandler = ServerConfig.getCustomGrantTypeHandler().newInstance();
    ...
    public boolean isExistingClient(String clientId) {
        ClientCredentials creds = db.findClientCredentials(clientId);
        if (creds != null) {
            return true;
        }
